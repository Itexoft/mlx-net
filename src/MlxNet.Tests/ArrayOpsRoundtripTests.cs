...
public unsafe class ArrayOpsRoundtripTests
{
        ...
        try
        {
            TestHelpers.WithStream(stream =>
            {
                TestHelpers.WithShape(
                    reshapedShape,
                    (shapePtr, shapeLength) =>
                    {
                        TestHelpers.Ok(MlxOps.Reshape(out var reshaped, original, shapePtr, shapeLength, stream), "reshape");
    ...
    public void Transpose_SwapsAxes()
    {
        ...
        try
        {
            TestHelpers.WithStream(stream =>
            {
                TestHelpers.Ok(MlxOps.Transpose(out var transposed, array, stream), "transpose");

                try
                {
                    TestHelpers.Ok(MlxArray.Eval(transposed), "eval transpose");
                    TestHelpers.Ok(MlxOps.Subtract(out var diff, transposed, expected, stream), "subtract");
    ...
    public void TakeAlong_GathersExpectedElements()
    {
        ...
        try
        {
            TestHelpers.WithStream(stream =>
            {
                TestHelpers.Ok(MlxOps.TakeAlongAxis(out var gathered, array, indices, 1, stream), "take_along_axis");
