...
public sealed class QuantizedLinear : Linear, IQuantized
{
    ...
    public QuantizedLinear(
        ...
        int groupSize = 64,
        int bits = 4,
        QuantizationMode mode = QuantizationMode.Affine) : this(
    ...
    public QuantizedLinear(Linear other, int groupSize = 64, int bits = 4, QuantizationMode mode = QuantizationMode.Affine) : this(
        other.Weight.Value.Copy(),
        other.Bias is null ? null : other.Bias.Value.Copy(),
        groupSize,
        bits,
        mode) { }

    public QuantizedLinear(MlxArrayHandle weight, MlxArrayHandle? bias, int groupSize, int bits, QuantizationMode mode) : base(
        QuantizeWeights(weight, groupSize, bits, mode, out var scales, out var biases),
        bias,
        false)
    {
