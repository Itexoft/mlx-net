...
public static class TensorFactory
{
    ...
    public static void Seed(ulong seed) => Interlocked.Exchange(ref seedCounter, unchecked((long)seed));
    ...
    public static MlxArrayHandle Scalar(float value, MlxDType dtype = MlxDType.MlxFloat32) => CreateScalar(value, dtype);
    ...
    public static unsafe MlxArrayHandle Full(float value, ReadOnlySpan<int> shape, MlxDType dtype = MlxDType.MlxFloat32)
    {
        ...
        try
        {
            ...
            {
                var status = MlxOps.Full(out var result, shapePtr, (nuint)shape.Length, fill, dtype, TensorUtilities.DefaultStream());
    ...
    /// <param name="shape">Tensor shape.</param>
    /// <param name="dtype">Target data type.</param>
    public static MlxArrayHandle Zeros(ReadOnlySpan<int> shape, MlxDType dtype = MlxDType.MlxFloat32) => Full(0f, shape, dtype);
    ...
    /// <param name="shape">Tensor shape.</param>
    /// <param name="dtype">Target data type.</param>
    public static MlxArrayHandle Ones(ReadOnlySpan<int> shape, MlxDType dtype = MlxDType.MlxFloat32) => Full(1f, shape, dtype);
    ...
    /// <param name="shape">Tensor shape.</param>
    /// <param name="dtype">Target data type.</param>
    public static unsafe MlxArrayHandle Uniform(float low, float high, ReadOnlySpan<int> shape, MlxDType dtype = MlxDType.MlxFloat32)
    {
    ...
    /// <param name="shape">Tensor shape.</param>
    /// <param name="dtype">Target data type.</param>
    public static unsafe MlxArrayHandle Normal(float mean, float std, ReadOnlySpan<int> shape, MlxDType dtype = MlxDType.MlxFloat32)
    {
        var key = NextRandomKey();
        ...
        try
        {
            fixed (int* shapePtr = shape)
            {
                var status = MlxRandom.Normal(out var result, shapePtr, (nuint)shape.Length, dtype, mean, std, key, TensorUtilities.DefaultStream());
    ...
    {
        var prob = Scalar(probability, MlxDType.MlxFloat32);
        var key = NextRandomKey();
        ...
        try
        {
            fixed (int* shapePtr = shape)
            {
                var status = MlxRandom.Bernoulli(out var result, prob, shapePtr, (nuint)shape.Length, key, TensorUtilities.DefaultStream());
    ...
    /// <param name="dtype">Target data type.</param>
    public static MlxArrayHandle Arange(int start, int stop, int step = 1, MlxDType dtype = MlxDType.MlxInt32)
    {
    ...
    /// <param name="step">Increment between values.</param>
    /// <param name="dtype">Target data type.</param>
    public static MlxArrayHandle Arange(float start, float stop, float step = 1f, MlxDType dtype = MlxDType.MlxFloat32)
    {
        var status = MlxOps.Arange(out var result, start, stop, step, dtype, TensorUtilities.DefaultStream());
        TensorUtilities.CheckStatus(status, "arange");
    ...
    {
        var seed = (ulong)Interlocked.Increment(ref seedCounter);
    ...
    /// <param name="value">Scalar value to materialize.</param>
    public static MlxArrayHandle ScalarLike(MlxArrayHandle reference, float value) => Scalar(value, MlxArray.DType(reference));
    ...
    public static MlxArrayHandle OnesLike(MlxArrayHandle reference) => Ones(reference.ShapeSpan(), MlxArray.DType(reference));

    /// <summary>
    /// Creates a tensor filled with zeros that matches the reference handle's dtype and size.
    /// </summary>
    /// <param name="reference">Tensor that defines the desired shape and data type.</param>
    public static MlxArrayHandle ZerosLike(MlxArrayHandle reference) => Zeros(reference.ShapeSpan(), MlxArray.DType(reference));

    private static MlxArrayHandle CreateScalar(float value, MlxDType dtype)
    {
        var scalar = MlxArray.NewFloat32(value);

        if (dtype == MlxDType.MlxFloat32)
            return scalar;
