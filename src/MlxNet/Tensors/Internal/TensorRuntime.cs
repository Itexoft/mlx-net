...
{
    private static nint sCpuDefaultStreamContext;

    private static nint sGpuDefaultStreamContext;
    ...
    private static MlxStreamHandle DefaultStream()
    {
        ...
        try
        {
            ...
            return type switch
            {
                MlxDeviceType.MlxCpu => GetDefaultCpuStream(),
                MlxDeviceType.MlxGpu => GetDefaultGpuStream(),
    ...
    private static MlxStreamHandle GetDefaultCpuStream() =>
        EnsureDefaultStream(ref sCpuDefaultStreamContext, MlxStream.DefaultCpuStreamNew, "default_cpu_stream_new");

    private static MlxStreamHandle GetDefaultGpuStream() =>
        EnsureDefaultStream(ref sGpuDefaultStreamContext, MlxStream.DefaultGpuStreamNew, "default_gpu_stream_new");

    private static MlxStreamHandle EnsureDefaultStream(
        ref nint cachedContext,
        Func<MlxStreamHandle> factory,
        string operation)
    {
        var current = Volatile.Read(ref cachedContext);
        if (current != 0)
            return new MlxStreamHandle { ctx = current };

        var created = factory();
        if (created.ctx == 0)
            throw new InvalidOperationException($"Failed to resolve MLX {operation}.");

        var published = Interlocked.CompareExchange(ref cachedContext, created.ctx, 0);
        if (published == 0)
            return created;

        CheckStatus(MlxStream.Free(created), "free_redundant_default_stream");
        return new MlxStreamHandle { ctx = published };
    }
